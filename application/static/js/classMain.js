...
var urlToGo = '/connect/'+userName+"/"+userEmoji
    ...
    if (this.currentFrame % this.frameSkip == 0) {
      ...
      if (dLips > (previousValues.lengthOfLip * this.smileLengthFactor) &&
        ...
        if (!IS_SMILING) {
          ...
          var urlToGo = '/smile/'+currentName+"/"+value
          fetch(urlToGo,options);
        }
        IS_SMILING = true;
      }
    ...
    if (this.frameCounterHandRaise % this.skipFrame === 0) {
      ...
      if (palm) {
        if (!IS_HAND_RAISED) {
          ...
          var urlToGo = '/handraise/'+currentName+"/"+value
          fetch(urlToGo,options);
        }
